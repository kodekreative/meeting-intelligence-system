...
import { getSecureTasksUrl, getLegacyTasksUrl } from './team-token.service.js'
...
// Helper to generate My Tasks URL (legacy, synchronous version)
// For new emails, use getSecureTasksUrl which generates secure token-based URLs
function getMyTasksUrl(assigneeName: string): string {
  return getLegacyTasksUrl(assigneeName)
}
...
/**
   ...
   *
   * Now uses secure tokens for "View My Tasks" links - each team member
   * gets a unique, non-guessable URL that only shows their tasks.
   */
  async sendFollowUpEmails(
    ...
    for (const [assigneeName, data] of actionItemsByAssignee.entries()) {
      // Generate secure token-based URL for this team member
      let secureTasksUrl: string
      try {
        secureTasksUrl = await getSecureTasksUrl(assigneeName)
      } catch (error) {
        console.warn(`Failed to generate secure URL for ${assigneeName}, using legacy URL:`, error)
        secureTasksUrl = getLegacyTasksUrl(assigneeName)
      }

      const html = this.generateFollowUpEmailHTML(assigneeName, data.items, senderName, secureTasksUrl)
      const text = this.generateFollowUpEmailText(assigneeName, data.items, senderName, secureTasksUrl)
